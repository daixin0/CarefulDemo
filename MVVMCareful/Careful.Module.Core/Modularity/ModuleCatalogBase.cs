...
namespace Careful.Module.Core.Modularity
{
    ...
    public class ModuleCatalogBase : IModuleCatalog
    {
            ...
            foreach (var moduleInfo in modules)
            {
                if (moduleInfo.DependsOn != null && moduleInfo.DependsOn.Except(moduleNames).Any())
                {
                    throw new ModularityException(
                        ...
                        string.Format(CultureInfo.CurrentCulture, "ModuleDependenciesNotMetInGroup", moduleInfo.ModuleName));
                }
            }
        }
        ...
        protected virtual void ValidateUniqueModules()
        {
            ...
            {
                throw new DuplicateModuleException(duplicateModule, string.Format(CultureInfo.CurrentCulture, "DuplicatedModule", duplicateModule));
            }
        }
        ...
        protected virtual void ValidateDependenciesInitializationMode()
        {
            ...
            if (moduleInfo != null)
            {
                throw new ModularityException(
                    moduleInfo.ModuleName,
                    string.Format(CultureInfo.CurrentCulture, "StartupModuleDependsOnAnOnDemandModule", moduleInfo.ModuleName));
            }
        }
