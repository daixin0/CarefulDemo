...
namespace Careful.Controls.DataGridControl.ConvertStringToLambda
{
    ...
    public static class DynamicExpressionParser
    {
        ...
        [PublicAPI]
        public static Expression ParseLambdaExpression([CanBeNull] ParsingConfig parsingConfig, bool createParameterCtor,Type sourceType, [NotNull] ParameterExpression[] parameters, [CanBeNull] Type resultType, [NotNull] string expression, params object[] values)
        {
            Check.NotNull(parameters, nameof(parameters));
            Check.HasNoNulls(parameters, nameof(parameters));
            Check.NotEmpty(expression, nameof(expression));

            var parser = new ExpressionParser(parameters, expression, values, parsingConfig, sourceType);
